...
import { fetchAntecedentesMedicos } from '../../api/api';

const { height, width } = Dimensions.get('window');

const AntecedentesMedicosListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchAntecedentesMedicos(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching antecedentes médicos:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('AntecedentesMedicosDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con antecedentes médicos</Text>
      </View>
      <TextInput
...
export default AntecedentesMedicosListScreen;