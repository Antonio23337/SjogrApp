...
import { fetchAlergias } from '../../api/api';

const { height, width } = Dimensions.get('window');

const AlergiasListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchAlergias(token);
        setData(response);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('AlergiasDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con alergias</Text>
      </View>
      <TextInput
...
export default AlergiasListScreen;