...
import { fetchHabitosNocivos } from '../../api/api';

const { height, width } = Dimensions.get('window');

const HabitosNocivosListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchHabitosNocivos(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching hábitos nocivos:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('HabitosNocivosDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con hábitos nocivos</Text>
      </View>
      <TextInput
...
export default HabitosNocivosListScreen;