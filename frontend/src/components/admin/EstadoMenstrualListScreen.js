...
import { fetchEstadoMenstrual } from '../../api/api';

const { height, width } = Dimensions.get('window');

const EstadoMenstrualListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchEstadoMenstrual(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching estado menstrual:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('EstadoMenstrualDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con estado menstrual</Text>
      </View>
      <TextInput
...
export default EstadoMenstrualListScreen;