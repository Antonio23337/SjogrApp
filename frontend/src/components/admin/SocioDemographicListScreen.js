...
import { fetchSocioDemographics } from '../../api/api';

const { height, width } = Dimensions.get('window');

const SocioDemographicListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchSocioDemographics(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching socio-demographics:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('SocioDemographicDetail', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios que han rellenado el formulario</Text>
      </View>
      <TextInput
...
export default SocioDemographicListScreen;
