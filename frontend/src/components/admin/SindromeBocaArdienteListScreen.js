...
import { fetchSindromeBocaArdiente } from '../../api/api';

const { height, width } = Dimensions.get('window');

const SindromeBocaArdienteListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchSindromeBocaArdiente(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching síndrome de la boca ardiente:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('SindromeBocaArdienteDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con síndrome de la boca ardiente</Text>
      </View>
      <TextInput
...
export default SindromeBocaArdienteListScreen;