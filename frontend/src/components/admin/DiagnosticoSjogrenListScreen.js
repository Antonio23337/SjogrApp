...
import { fetchDiagnosticoSjogren } from '../../api/api';

const { height, width } = Dimensions.get('window');

const DiagnosticoSjogrenListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchDiagnosticoSjogren(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching diagnóstico de Sjogren:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('DiagnosticoSjogrenDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con diagnóstico de Sjogren</Text>
      </View>
      <TextInput
...
export default DiagnosticoSjogrenListScreen;