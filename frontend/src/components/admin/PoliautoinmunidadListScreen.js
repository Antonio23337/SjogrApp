...
import { fetchPoliautoinmunidad } from '../../api/api';

const { height, width } = Dimensions.get('window');

const PoliautoinmunidadListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchPoliautoinmunidad(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching poliautoinmunidad:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('PoliautoinmunidadDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con poliautoinmunidad</Text>
      </View>
      <TextInput
...
export default PoliautoinmunidadListScreen;
