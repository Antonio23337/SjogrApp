...
import { fetchESSPRI } from '../../api/api';

const { height, width } = Dimensions.get('window');

const ESSPRIListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchESSPRI(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching ESSPRI:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('ESSPRIDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con ESSPRI</Text>
      </View>
      <TextInput
...
export default ESSPRIListScreen;