...
import { fetchXerostomia } from '../../api/api';

const { height, width } = Dimensions.get('window');

const XerostomiaListScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetchXerostomia(token);
        setData(response);
        setFilteredData(response);
      } catch (error) {
        console.error('Error fetching xerostomia:', error);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate('XerostomiaDetailScreen', { token, id: item.id })}
  ...
  return (
    <View style={styles.container}>
      <View style={styles.titleContainer}>
        <Text style={styles.title}>Usuarios con xerostomía</Text>
      </View>
      <TextInput
...
export default XerostomiaListScreen;